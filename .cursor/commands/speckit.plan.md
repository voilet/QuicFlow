---
description: 使用计划模板执行实现规划工作流以生成设计文档
---
...
1. **设置**：从仓库根目录运行 `.specify/scripts/bash/setup-plan.sh --json` 并解析 JSON 以获取 FEATURE_SPEC、IMPL_PLAN、SPECS_DIR、BRANCH。对于参数中的单引号（如"I'm Groot"），使用转义语法：例如 'I'\''m Groot'（或者如果可能使用双引号："I'm Groot"）。
...
3. **执行计划工作流**：按照 IMPL_PLAN 模板中的结构：
   ...
   - 从宪章填写宪章检查章节
   - 评估门控（如果违规未合理说明则报错）
   - 阶段 0：生成 research.md（解决所有需要澄清的内容）
